...
def update_resource(name)
  ...
  if !op_defaults.nil? &&
    ...
    if ops.is_a? Hash
    ...
    elsif ops.current_normal.key?("monitor") &&
          !ops.current_default.fetch("monitor").key?("on-fail")
      ...
      ops["monitor"] = op_defaults["monitor"]
          ...
          ops.current_default.fetch("monitor").key?("on-fail")
      # Delete 'normal' value because 'default' has priority
      ops.current_normal.fetch("monitor").delete("on-fail")
    end
